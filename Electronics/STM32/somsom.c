...
uint8_t pre_transmit_USB[3] = {0,0,0};
uint8_t transmit_USB[14] = {0};
uint8_t receive[17];
...
int CheckSum(int);
void is_delay(void){ for(int volatile unsigned k=0; k<20000; k++);}
void USB_Transmit_feedback(uint8_t* buf, uint8_t length);
...
void CDC_On_Receive(uint8_t* buffer, uint32_t* length){
	...
	case 167:
		...
		motors[0].Kp = *pointing / 100.0;
		motors[1].Kp = *pointing / 100.0;
		motors[2].Kp = *pointing / 100.0;
		pointing++;
		motors[0].Ki = *pointing/100.0;
		motors[1].Ki = *pointing/100.0;
		motors[2].Ki = *pointing/100.0;
		pointing++;
		motors[0].Kd = *pointing/100.0;
		motors[1].Kd = *pointing/100.0;
		motors[2].Kd = *pointing/100.0;
		break;
	}
}
...
//		error = the_speed*timer_s*0.5 - enc_delta;
		error = motors[k].max_enc*timer_s*motors[k].USB_value - enc_delta;

		if(error > 2 || error < -2){
			P = motors[k].Kp*error;

			I = (motors[k].integration + error)*motors[k].Ki;
			motors[k].integration = motors[k].integration + error;

			D = (error-motors[k].prev_err)/timer_s*motors[k].Kd;
			motors[k].prev_err = error;

			motors[k].speed = motors[k].speed + (D+I+P)/(motors[k].max_enc*timer_s/0.5);
		...
		pre_transmit_USB[k] = motors[k].speed*100;
	...
	//USB_Transmit_feedback(&pre_transmit_USB, 3);
	//GPIOB->ODR ^= BIT4;
	TIM7->SR = 0;
}
...
float* USB_command_translate(uint32_t message_length, uint8_t num_of_vals){
	...
	// Amount of counted numbers must match the input parameter
...
void USB_Transmit_feedback(uint8_t* buf, uint8_t length){
	uint8_t pos_count = 0;
	//transmit_USB[pos_count] = 60;			pos_count++;
	for(int k=0; k<length; k++){
		if(*buf == 100){
			transmit_USB[pos_count] = 1+48; pos_count++;
			transmit_USB[pos_count] = 0+48; pos_count++;
			transmit_USB[pos_count] = 0+48; pos_count++;
			transmit_USB[pos_count] = 58  ; pos_count++;
		}
		else if(*buf >9){
			transmit_USB[pos_count] = *buf/10 + 48; pos_count++;
			transmit_USB[pos_count] = *buf%10 + 48; pos_count++;
			transmit_USB[pos_count] = 58 	 ; pos_count++;
		}
		else if (*buf <10 && *buf >= 0){
			transmit_USB[pos_count] = *buf + 48	; pos_count++;
			transmit_USB[pos_count] = 58  		; pos_count++;
		}
		else{
			transmit_USB[pos_count] = 0 + 48	; pos_count++;
	...
	//transmit_USB[pos_count] = 62  		; pos_count++;
	CDC_Transmit_FS(&transmit_USB, pos_count);
